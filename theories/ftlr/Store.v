...
Qed.
                     ...
                     iExists p'. do 2 (iSplit; [auto | ]).
                       { iAlways.
                         ...
                         - iIntros "[HA [% %]]". iSplitL "HA"; auto.
                           iPureIntro; split.
                           + by eelim (region_state_nwl_monotone_nl _ _ y H5 Hfuture H4).
    ...
    (*           (* Before we can do that, we need to reestablish interp and monotonicity for the new value *) *)
    (*           iAssert ((fixpoint interp1) W (inl z)) as "#Hvsrc". *)
    (*           {  by rewrite (fixpoint_interp1_eq _ (inl z)) /=. } *)
    (*           (* Change the below to a match fs*) *)
    (*           iAssert (if decide (ρ' = Temporary ∧ pwl p1 = true) *)
    (*                    then *)
    (*                      future_pub_mono *)
    (*                        (λ Wv : prodO (leibnizO (STS * STS)) (leibnizO Word), *)
    (*                                ((fixpoint interp1) Wv.1) Wv.2) (inl z) *)
    (*                    else *)
    (*                      future_priv_mono *)
    (*                        (λ Wv : prodO (leibnizO (STS * STS)) (leibnizO Word), *)
    (*                                ((fixpoint interp1) Wv.1) Wv.2) (inl z) *)
    (*                   ) as "HvPM". *)
    (*           {destruct (decide (ρ' = Temporary ∧ pwl p1 = true)); iAlways; simpl. *)
    (*            - iIntros (W0 W1) "% HIW0". *)
    (*                by iApply interp_monotone. *)
    (*            - iIntros (W0 W1) "% HIW0". *)
    (*                by iApply interp_monotone_nl. *)
    ...
    (*           iDestruct (region_close_next with "[$Hr $Ha0 $Ha2a1 $Hstate' $Hfuture]") as "Hr"; eauto. *)
    (*           { apply not_elem_of_cons; split; [auto|apply not_elem_of_nil]. } *)
