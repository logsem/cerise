...
Record initial_prog := MkInitialProg {
  prog_start: Addr;
  prog_end: Addr;
  prog: list Word;

  prog_size:
    (prog_start + length prog)%a = Some prog_end;
}.

Definition prog_region (P: initial_prog): gmap Addr Word :=
  mkregion (prog_start P) (prog_end P) (prog P).

Definition is_initial_registers (P: initial_prog) (reg: gmap RegName Word) :=
  reg !! PC = Some (inr (RWX, prog_start P, prog_end P, prog_start P)) ∧
  (∀ (r: RegName), r ∉ ({[ PC ]} : gset RegName) →
     ∃ (w:Word), reg !! r = Some w ∧ is_cap w = false).

Lemma initial_registers_full_map (P: initial_prog) reg :
...
Definition is_initial_memory (P: initial_prog) (m: gmap Addr Word) :=
  prog_region P ⊆ m.
...
Section Adequacy.
  ...
  Lemma template_adequacy' (m m': Mem) (reg reg': Reg) (es: list cap_lang.expr):
    ...
    (∀ `{memG Σ, regG Σ, logrel_na_invs Σ} rmap,
     ...
     ⊢ inv invN (minv_sep I)
       ∗ PC ↦ᵣ inr (RWX, prog_start P, prog_end P, prog_start P)
  ...
  Proof.
    ...
    { iNext. unfold minv_sep. iExists prog_in_inv. iFrame. iPureIntro.
      assert (dom (gset Addr) prog_in_inv = minv_dom I) as Hi_dom.
      { eapply set_eq. intros a. rewrite /prog_in_inv.
        set prog_addrs := region_addrs (prog_start P) (prog_end P).
        set inv_addrs := filter (λ a, a ∈ minv_dom I) prog_addrs.
        set inv_addrs_s := (list_to_set inv_addrs : gset Addr).
        rewrite (dom_filter_L _ _ inv_addrs_s).
        2: { intros a'. rewrite /inv_addrs_s /inv_addrs /prog_addrs.
             rewrite elem_of_list_to_set elem_of_list_filter. split.
             { intros [? ?].
               assert (is_Some (prog_region P !! a')) as [? ?].
               { apply elem_of_gmap_dom. rewrite dom_mkregion_eq. 2: by apply prog_size.
                 rewrite elem_of_list_to_set //. }
               eexists. split; eauto. }
             { intros [? (? & ?)]. split; auto.
               eapply in_dom_mkregion, elem_of_gmap_dom. eauto. } }
        rewrite /inv_addrs_s /inv_addrs elem_of_list_to_set elem_of_list_filter.
        split; intros; destruct_and?; split_and?; eauto.
        pose proof (proj1 (elem_of_subseteq _ _) HIdom a) as HH.
        rewrite elem_of_list_to_set in HH |- * => HH. eauto. }
      split; auto; []. eapply minv_sub_restrict; [ | | eapply HI].
      by rewrite Hi_dom //. transitivity (prog_region P); auto. rewrite /prog_in_inv.
