...
From cap_machine.rules_binary Require Import rules_binary_base rules_binary_GetSealed.
...
Section fundamental.
  ...
  Lemma GetSealed_spec_determ r dst src regs regs' retv retv' :
    GetSealed_spec r dst src regs retv ->
    GetSealed_spec r dst src regs' retv' ->
  ...
  Lemma get_sealed_case (r : prodO (leibnizO Reg) (leibnizO Reg)) (p : Perm)
        (b e a : Addr) (w w' : Word) (dst : RegName) (src : RegName) (P : D):
    ftlr_instr r p b e a w w' (GetSealed dst src) P.
  Proof.
    ...
    iApply (wp_GetSealed with "[$Ha $Hmap]"); eauto.
    ...
    iMod (step_GetSealed _ [SeqCtx] with "[$Ha' $Hsmap $Hs $Hspec]") as (retv' regs'') "(Hs' & Hs & Ha' & Hsmap) /=";[rewrite Heqw in Hi|..];eauto.
    ...
    specialize (GetSealed_spec_determ _ _ _ _ _ _ _ HSpec HSpec') as [Hregs <-].
