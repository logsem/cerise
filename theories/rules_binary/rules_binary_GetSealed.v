From cap_machine Require Export rules_binary_base rules_GetSealed.
...
Section cap_lang_spec_rules.
  ...
  Lemma step_GetSealed Ep K pc_p pc_b pc_e pc_a w dst src regs :
    ...
    ={Ep}=∗ ∃ retv regs', ⤇ fill K (of_val retv) ∗ ⌜ GetSealed_spec regs dst src regs' retv ⌝ ∗ pc_a ↣ₐ w ∗ ([∗ map] k↦y ∈ regs', k ↣ᵣ y).
  Proof.
    ...
    assert (exec_opt (GetSealed dst src) (σr, σm) = updatePC (update_reg (σr, σm) dst (WInt (if is_sealed wsrc then 1%Z else 0%Z)))) as HH.
    {  rewrite /= Hwsrc'. unfold is_sealed; destruct_word wsrc; auto. }
    rewrite HH in Hstep. rewrite /update_reg /= in Hstep.

    destruct (incrementPC (<[ dst := WInt (if is_sealed wsrc then 1%Z else 0%Z) ]> regs)) as [regs''|] eqn:Hregs';
    ...
    simplify_pair_eq.
    iMod ((regspec_heap_update_inSepM _ _ _ dst (WInt (if is_sealed wsrc then 1%Z else 0%Z))) with "Hown Hmap") as "[Hown Hmap]"; eauto.
