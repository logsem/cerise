...
Section lse.
  ...
  (* (* a macro for preparing the stack for a stack call (note that no local state is stored) *) *)
  (* (* prepares the stack up until stack capability restrict *) *)
  (* Definition scall_prepare_prog off_after := *)
  ...
  (*     (* push old pc *) *)
  (*     move_r r_t1 PC; *)
  (*     lea_z r_t1 off_after; (* 64 = offset to "after" in contiguous program *) *)
  (*     lea_z r_stk 1; *)
  (*     store_r r_stk r_t1; *)
  (*     (* push stack pointer *) *)
  (*     lea_z r_stk 1; *)
  ...
  (* Definition scall_prepare (a : list Addr) p (off_after : Z) : iProp Σ := *)
  (*   if ((length a) =? (length (scall_prepare_prog off_after)))%nat then *)
  (*     ([∗ list] k↦a_i;w_i ∈ a;(scall_prepare_prog off_after), a_i ↦ₐ[p] w_i)%I *)
  (*   else *)
  (*     False%I. *)

  (* Lemma scall_prepare_spec (a a_r_l : list Addr) off_after r *)
  (*       (a_last a_first a_after : Addr) p p' g b e p_r p_r' g_r b_r e_r a_r e_r' a' φ : *)
  (*   (∀ i ai aj, a !! i = Some ai → a !! (i + 1) = Some aj → (ai + 1)%a = Some aj) → *)
  (*   a !! 0 = Some a_first → list.last a = Some a_last → *)
  (*   (∀ i ai aj, a_r_l !! i = Some ai → a_r_l !! (i + 1) = Some aj → (ai + 1)%a = Some aj) → *)
  (*   a_r_l !! 0 = Some a_r → list.last a_r_l = Some e_r' → *)
  (*   a_r_l = region_addrs a_r e_r' → *)
  (*   (* We will assume the stack has write local capabilities *) *)
  (*   p_r = RWLX ∨ p_r = RWL → *)
  (*   (a !! 11 = Some a_after) → *)
  ...
  (*    (scall_prepare a p' off_after  *)
  (*   ∗ ▷ PC ↦ᵣ inr ((p,g),b,e,a_first) *)
  (*   ∗ ▷ r ↦ᵣ inr ((p_r,g_r),b_r,e_r,a_r) *)
  (*   ∗ ▷ (∃ w1, r_t1 ↦ᵣ w1) *)
  (*   ∗ ▷ (∃ w2, r_t2 ↦ᵣ w2) *)
  (*   ∗ ▷ (∃ w3, r_t3 ↦ᵣ w3) *)
  (*   ∗ ▷ (∃ ws, [∗ list] y1;y2 ∈ a_r_l;ws, y1 ↦ₐ[p_r'] y2) *)
  (*   ∗ ▷ (PC ↦ᵣ inr ((p,g),b,e,a') *)
  (*           ∗ r_t1 ↦ᵣ inl 0%Z *)
  (*           ∗ r_t2 ↦ᵣ inl 0%Z *)
  (*           ∗ r_t3 ↦ᵣ inl 0%Z *)
  (*           ∗ r ↦ᵣ inr ((p_r,g_r),b_r,e_r,e_r') *)
  (*           ∗ ([∗ list] y1;y2 ∈ a_r_l;[(inl w_1); *)
  (*                                      (inl w_2); *)
  (*                                      (inl w_3); *)
  (*                                      (inl w_4a); *)
  (*                                      (inl w_4b); *)
  (*                                      (inl w_4c)], y1 ↦ₐ[p_r'] y2) *)
  (*           ∗ scall_prepare a p' off_after  -∗  *)
  (*           WP Seq (Instr Executable) {{ φ }}) *)
  (*   ⊢ WP Seq (Instr Executable) {{ φ }})%I.   *)
  (* Proof. *)
  (*   iIntros (Hnext Ha_first Ha_last Hnextr Ha_firstr Ha_lastr Heq *)
  (*                  Hp_r Ha_after Hvpc1 Hvpc2 Hfl1 Hfl2 Ha_last' He_r') *)
  (*           "(Hprep & >HPC & >Hr & >Hr_t1 & >Hr_t2 & >Hr_t3 & >Hstack_own & Hφ)". *)
  (*   iDestruct "Hr_t1" as (w1) "Hr_t1". *)
  (*   iDestruct "Hr_t2" as (w2) "Hr_t2". *)
  (*   iDestruct "Hr_t3" as (w3) "Hr_t3". *)
  (*   iAssert (⌜((length a) =? (length (scall_prepare_prog _)) = true)%nat⌝)%I as %Hlen. *)
  (*   { destruct (((length a) =? (length (scall_prepare_prog _)))%nat) eqn:Hlen; auto. *)
  (*     rewrite /scall_prepare Hlen. by iApply bi.False_elim. } *)
  (*   rewrite /scall_prepare Hlen /mclear_prog; simpl in Hlen. apply beq_nat_true in Hlen. *)
  (*   destruct a as [_ | a0 al]; [inversion Hlen|]; simpl; inversion Ha_first.  *)
  (*   do 8 (destruct al as [_ | ? al]; [inversion Hlen|]). *)
  (*   iDestruct "Hstack_own" as (ws) "Hstack_own". *)
  (*   assert ((a_r <= e_r')%a) as Hle. *)
  (*   { rewrite /incr_addr in He_r'. *)
  (*     rewrite /incr_addr in He_r'. *)
  (*     destruct (Z_le_dec (a_r + 8)%Z MemNum); inversion He_r'. *)
  (*     destruct a_r. rewrite /le_addr /=. lia. } *)
  (*   assert (length (region_addrs a_r e_r') = 9) as Hslen. *)
  (*   { rewrite region_addrs_length; auto.  *)
  (*     rewrite /region_size. *)
  (*     rewrite /incr_addr in He_r'. *)
  (*     destruct (Z_le_dec (a_r + 8)%Z MemNum); inversion He_r'. simpl. lia. *)
  (*   } *)
  (*   iDestruct (big_sepL2_length with "Hstack_own") as %Hslen'. *)
  (*   rewrite Heq Hslen in Hslen'. rewrite -Heq in Hslen.  *)
  (*   destruct a_r_l;inversion Hslen. inversion Ha_firstr. *)
  (*   do 8 (destruct a_r_l;[inversion Hslen|]). destruct a_r_l;[|inversion Hslen]. *)
  (*   inversion Ha_lastr. *)
  (*   iDestruct "Hprep" as "[Hi Hf2]". *)
  (*   destruct ws as [_ | wa ws]; first inversion Hslen'.  *)
  (*   iDestruct "Hstack_own" as "[Ha Hstack]". *)
  (*   iApply (push_z_spec a_first a a1 _ _ _ _ _ _ _ b e a_first a φ); *)
  (*     try addr_succ; try apply PermFlows_refl; *)
  (*       first (split; eauto; apply isCorrectPC_bounds with a_first a_last; eauto; split).  *)
  (*   (*   [split; eauto; apply isCorrectPC_bounds with a_lo a_hi; eauto; split; done|]; *) *)
  (*   (*   iFrame; iNext; iIntros ptrn *) *)

    
  (*   wp_push_z "Hstack_own" a_first a_last b_r e_r a_r a9 φ a_first a a1 ws "Hprep" *)
  (*             "(HPC & _ & Hr_stk & Ha120)". *)
