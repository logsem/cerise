...
\uncover<5->{We can take advantage of Iris' step-indexed model and invariants to mechanize step-indexed Kripke logical relations with recursive worlds in a succinct and elegant way.}
...
\uncover<2->{
$$  \mathcal{V}\only<6->{{\color{red} (W)}}((\text{\tiny{RW}},g),b,e,a) \triangleq~ \bigsep_{a \in [b,e]} \fbox{$\exists w, a \mapsto_a [RW] w \sep \mathcal{V}\only<5->{{\color{red} (W)}}(w)$}$$}
