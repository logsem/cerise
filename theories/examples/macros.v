...
Section macros.
  ...
  Lemma fetch_spec f a pc_p pc_b pc_e a_first a_last b_link e_link a_link entry_a wentry φ w1 w2 w3:
    ...
    ∗ ▷ (PC ↦ᵣ inr (pc_p,pc_b,pc_e,a_last) ∗ fetch f a
  ...
  Proof.
    ...
    iApply (wp_load_success_same with "[$HPC $Hi $Hr_t1 Ha_entry]");
      [exact wentry|apply decode_encode_instrW_inv|iCorrectPC a_first a_last|auto|apply Hlink|..].
  ...
  Definition assert_r_z a f_a r z :=
    ([∗ list] a_i;w_i ∈ a;(assert_r_z_instrs f_a r z), a_i ↦ₐ w_i)%I.
  ...
  Lemma assert_r_z_success a f_a r z pc_p pc_b pc_e a_first a_last
    ...
    ∗ ▷ (r_t1 ↦ᵣ inl 0%Z ∗ r_t2 ↦ᵣ inl 0%Z ∗ r_t3 ↦ᵣ inl 0%Z ∗ r ↦ᵣ inl 0%Z
         ∗ PC ↦ᵣ inr (pc_p,pc_b,pc_e,a_last) ∗ assert_r_z a f_a r z
  ...
  Proof.
    ...
    iApply (wp_add_sub_lt_success_dst_z with "[$HPC $Hi $Hr]");
      [apply decode_encode_instrW_inv|right;left;eauto|iContiguous_next Hcont_rest 0|iCorrectPC link a_last|..].
  ...
  Lemma assert_r_z_fail a f_a r z pc_p pc_b pc_e a_first a_last z_r
    ...
    isCorrectPC_range RX f_b f_e f_a_first f_a_last →
    contiguous_between a' f_a_first f_a_last →
    (f_a_first + (length a'))%a = Some a_env ->
    withinBounds (RX,f_b,f_e,a_env) = true ->
    (* flag assumptions *)
    withinBounds (flag_p,flag_b,flag_e,flag_a) = true ∧ writeAllowed flag_p = true ->
    (* condition for assertion success *)
    (z_r ≠ z) ->
    ...
    ∗ ▷ (∃ w, flag_a ↦ₐ w) (* the assertion flag *)
    (* registers *)
    ∗ ▷ r ↦ᵣ inl z_r
    ∗ ▷ (∃ w, r_t1 ↦ᵣ w)
    ∗ ▷ (∃ w, r_t2 ↦ᵣ w)
    ...
    {{{ RET FailedV; r_t1 ↦ᵣ inl 0%Z ∗ r_t2 ↦ᵣ inl 0%Z ∗ r_t3 ↦ᵣ inl 0%Z ∗ (∃ z, r ↦ᵣ inl z ∧ ⌜z ≠ 0⌝)
         ∗ PC ↦ᵣ inr (RX, f_b, f_e,^(f_a_last + (-1))%a)
  ...
  Proof.
    iIntros (Hvpc Hcont Hwb Htable Hvpc' Hcont' Henv Henvwb [Hwb' Hwa] Hfailure φ)
    ...
    iApply (wp_jnz_success_jmp with "[$HPC $Hi $Hr_t1 $Hr]");
      [apply decode_encode_instrW_inv|iCorrectPC link a_last|simpl;revert Hfailure;clear;intros Hne Hcontr;simplify_eq;lia|..].
    ...
    iPrologue "Hprog'".
    iApply (wp_move_success_reg_fromPC with "[$HPC $Hi $Hr_t1]");
      [apply decode_encode_instrW_inv|iCorrectPC f_a_first f_a_last|iContiguous_next Hcont' 0|auto|..].
    ...
    iApply (wp_lea_success_z with "[$HPC $Hi $Hr_t1]");
      [apply decode_encode_instrW_inv|iCorrectPC f_a_first f_a_last|iContiguous_next Hcont' 1|apply Hlea|auto..].
    iEpilogue "(HPC & Hi & Hr_t1)". iCombine "Hi" "Hprog_done'" as "Hprog_done'".
    (* load r_t1 r_t1 *)
    destruct a';[inversion Hlength'|].
    iPrologue "Hprog".
    ...
    iApply (wp_load_success_same with "[$HPC $Hi $Hr_t1 Ha_env]");
      [|apply decode_encode_instrW_inv|iCorrectPC f_a_first f_a_last|split;auto|iContiguous_next Hcont' 2|
    ...
    iApply (wp_store_success_z with "[$HPC $Hi $Hr_t1 $Hflag]");
      [apply decode_encode_instrW_inv|iCorrectPC f_a_first f_a_last|iContiguous_next Hcont' 3|
       split;auto|].
    ...
    iPrologue "Hprog".
    iApply (wp_move_success_z with "[$HPC $Hi $Hr_t1]");
      [apply decode_encode_instrW_inv|iCorrectPC f_a_first f_a_last|iContiguous_next Hcont' 4|].
    iEpilogue "(HPC & Hi & Hr_t1)"; iCombine "Hi" "Hprog_done" as "Hprog_done".
    (* Halt *)
    destruct a';[|inversion Hlength'].
  ...
  Lemma malloc_spec φ size cont a pc_p pc_b pc_e a_first a_last
    ...
    (* continuation *)
    ∗ ▷ (PC ↦ᵣ inr (pc_p,pc_b,pc_e,a_last) ∗ malloc f_m size a
         ∗ pc_b ↦ₐ inr (RO,b_link,e_link,a_link)
         ∗ a_entry ↦ₐ inr (E,b_m,e_m,b_m)
         (* the newly allocated region *)
         ∗ (∃ (b e : Addr),
  ...
  Lemma rclear_spec (a : list Addr) (r: list RegName) (rmap : gmap RegName Word) p b e a1 an φ :
    ...
    ∗ ▷ PC ↦ᵣ inr (p,b,e,a1)
    ∗ ▷ rclear a r
    ∗ ▷ (PC ↦ᵣ inr (p,b,e,an) ∗ ([∗ map] r_i↦_ ∈ rmap, r_i ↦ᵣ inl 0%Z)
            ∗ rclear a r -∗
            WP Seq (Instr Executable) {{ φ }})
    ⊢ WP Seq (Instr Executable) {{ φ }}.
  Proof.
  ...
  Lemma mclear_iter_spec (a1 a2 a3 a4 a5 a6 b_r e_r a_r (* e_r' *) : Addr) ws (z : nat)
      ...
      ∧ isCorrectPC (inr (p,b,e,a3))
      ∧ isCorrectPC (inr (p,b,e,a4))
      ∧ isCorrectPC (inr (p,b,e,a5))
      ∧ isCorrectPC (inr (p,b,e,a6)) →
        (a1 + 1)%a = Some a2
      ∧ (a2 + 1)%a = Some a3
      ∧ (a3 + 1)%a = Some a4
      ∧ (a4 + 1)%a = Some a5
      ∧ (a5 + 1)%a = Some a6 →
        ((b_r + z < e_r)%Z → withinBounds (p_r,b_r,e_r,a_r) = true) →
        writeAllowed p_r = true →
        (* (e_r + 1)%a = Some e_r' → *)
        (b_r + z)%a = Some a_r →
      ([[a_r,e_r]] ↦ₐ [[ws]]
     ∗ ▷ PC ↦ᵣ inr (p,b,e,a1)
     ∗ ▷ rt ↦ᵣ inr (p_r,b_r,e_r,a_r)
     ∗ ▷ rt1 ↦ᵣ inl (b_r + z)%Z
     ∗ ▷ rt2 ↦ᵣ inl ((z_of e_r) - 1)%Z
     ∗ ▷ (∃ w, rt3 ↦ᵣ w)
     ∗ ▷ rt4 ↦ᵣ inr (p, b, e, a_end)
     ∗ ▷ rt5 ↦ᵣ inr (p, b, e, a1)
     ∗ ▷ ([∗ list] a_i;w_i ∈ [a1;a2;a3;a4;a5;a6];[lt_r_r rt3 rt2 rt1;
                                                  ...
                                                  add_r_z rt1 rt1 1;
                                                  jmp rt5], a_i ↦ₐ w_i)
     ∗ ▷ (PC ↦ᵣ updatePcPerm (inr (p,b,e,a_end))
             ∗ [[ a_r , e_r ]] ↦ₐ [[ region_addrs_zeroes a_r e_r ]]
             ∗ (∃ a_r, rt ↦ᵣ inr (p_r, b_r, e_r, a_r))
             ∗ rt5 ↦ᵣ inr (p, b, e, a1)
             ∗ a3 ↦ₐ store_z rt 0
             ∗ a4 ↦ₐ lea_z rt 1
  ...
  Proof.
    ...
    - assert (Z.b2z (e_r - 1 <? b_r + z)%Z = 0%Z) as Heq0.
      ...
      ; iFrame. all: auto.
      + by rewrite Ha_r'.
      + assert (updatePcPerm (inr (p, b, e, a1)) = (inr (p, b, e, a1))).
    ...
    - assert (Z.b2z (e_r - 1 <? b_r + z)%Z = 1%Z) as Heq0.
      ...
      iDestruct "Hprog" as "[Hi Hprog]".
      iApply (wp_bind (fill [SeqCtx])).
      iApply (wp_jnz_success_jmp _ rt4 rt3 _ _ _ a2 _ _ (inl 1%Z) with "[HPC Hi Hr_t3 Hr_t4]"); first apply decode_encode_instrW_inv; eauto.
      iFrame. iEpilogue "(HPC & Ha2 & Hr_t4 & Hr_t3)".
  ...
  Proof.
    ...
    (* getb r_t1 r_t4 *)
    iPrologue "Hprog".
    iApply (wp_Get_success _ _ r_t1 r_t4 _ _ _ a0 _ _ _ a1 with "[$HPC $Hi $Hr_t1 $Hr_t4]");
    ...
    (* geta r_t2 r_t4 *)
    iPrologue "Hprog".
    iApply (wp_Get_success _ _ r_t2 r_t4 _ _ _ a1 _ _ _ a2 with "[HPC Hi Hr_t2 Hr_t4]");
    ...
    (* gete r_t2 r_t4 *)
    iPrologue "Hprog".
    iApply (wp_Get_success _ _ r_t5 r_t4 _ _ _ a4 _ _ _ a5 with "[HPC Hi Hr_t5 Hr_t4]"); try iFrame;
    ...
    iPrologue "Hprog".
    assert (p ≠ E) as Hpne.
    { have: (isCorrectPC (inr (p, b, e, a_first))).
  ...
  Lemma reqperm_spec r perm a w pc_p pc_b pc_e a_first a_last φ :
    ...
    ∗ ▷ (if isPermWord w perm then
           ∃ b e a', ⌜w = inr (perm,b,e,a')⌝ ∧
          (PC ↦ᵣ inr (pc_p,pc_b,pc_e,a_last) ∗ reqperm r (encodePerm perm) a ∗
            r ↦ᵣ inr (perm,b,e,a') ∗ r_t1 ↦ᵣ inl 0%Z ∗ r_t2 ↦ᵣ inl 0%Z
            -∗ WP Seq (Instr Executable) {{ φ }})
        else φ FailedV)
    ⊢
      WP Seq (Instr Executable) {{ φ }}.
  Proof.
    ...
    destruct c,p,p. iSimpl in "Hr_t1".
    ...
    - (* p is perm *)
      ...
      iSimpl in "Hcont". rewrite isPerm_refl. iDestruct "Hcont" as (b e0 a' Heq) "Hφ". inversion Heq; subst.
      iPrologue "Hprog".
      ...
      iEpilogue "(HPC & Hi & Hr_t2)"; iCombine "Hi" "Hprog_done" as "Hprog_done".
      assert (updatePcPerm (inr (pc_p, pc_b, pc_e, a10)) = (inr (pc_p, pc_b, pc_e, a10))) as ->.
    ...
    - assert (p ≠ perm) as HneP.
      ...
      (* fail *)
      iPrologue "Hprog".
      assert (updatePcPerm (inr (pc_p, pc_b, pc_e, a9)) = (inr (pc_p, pc_b, pc_e, a9))) as ->.
      { destruct pc_p; auto. congruence. }
      iApply (wp_fail with "[$HPC $Hi]");
        [apply decode_encode_instrW_inv|iCorrectPC a_first a_last|].
  ...
  *)
  ...
  Proof.
    ...
    iApply (wp_store_success_z with "[$HPC $Hi $Hr_t1 $Ha4]");
      [apply decode_encode_instrW_inv|iCorrectPC a_first a_last|iContiguous_next Hcont 8|..].
    ...
    iApply (wp_store_success_reg with "[$HPC $Hi $Ha6 $Hrcode $Hr_t1]");
      [apply decode_encode_instrW_inv|iCorrectPC a_first a_last|iContiguous_next Hcont 12|..].
    ...
    iApply (wp_store_success_reg with "[$HPC $Hi $Ha7 $Hrdata $Hr_t1]");
      [apply decode_encode_instrW_inv|iCorrectPC a_first a_last|iContiguous_next Hcont 15|..].
