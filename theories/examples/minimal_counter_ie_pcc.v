...
Section counter.
  ...
  Lemma counter_init_spec (a_init a_data: Addr) (wadv w1 wdat0 : Word) φ :
   ...
   ⊢ (( PC ↦ᵣ WCap RX a_init a_end a_init
      ...
      ∗ ▷ (  PC ↦ᵣ updatePcPerm wadv
           ∗ idc ↦ᵣ WCap IEpcc a_data a_data_end a_secret
           ∗ r_t1 ↦ᵣ WInt 0
           ∗ r_t31 ↦ᵣ wadv
  ...
  Proof.
    ...
    iIntros (Hcont_code Hcont_data)
      "(HPC & Hidc & Hr1 & Hr31 & Hdat0 & Hprog & Hφ)".
    iGo "Hprog".
  ...
  Lemma counter_full_run_spec (a_init a_data: Addr) b_adv e_adv w1 wdat0 rmap adv :
    let a_code := (a_init ^+ code_off)%a in
    let a_end := (a_init ^+ (code_off + end_off))%a in
    let a_secret := (a_data ^+ secret_off)%a in
    let a_data_end := (a_data ^+ data_end_off)%a in
  ...
  Proof.
    iIntros (? ? ? ? Hcont_code Hcont_data Hrdom Hadv_closed Headv)
      "(#HI & HPC & Hidc & Hr1 & Hr31 & Hrmap & Hinit & Hcode & Hdata & Hadv & Hna)".
    ...
    iNext. iIntros "(HPC & Hidc & Hr1 & Hr31 & Hdat & Hinit)".
    ...
    iAssert (interp (WCap IEpcc a_data a_data_end a_secret)) as "#Hcode_safe".
    { rewrite /interp /= (fixpoint_interp1_eq (WCap IEpcc _ _ _)) /=.
      iIntros (Hinbounds).
      iExists (cap_eq RX a_init a_end a_code).
      iSplit ; first (iApply cap_eq_persistent).
      iSplit; first (iApply inv_cap_eq; iFrame "#").

      iIntros (wcode regs) ; iNext ; iModIntro.
      iIntros "%Hprog ([%Hrfull #Hr_interp] & Hrmap & Hna)".
      simplify_eq.
...
Proof.
  ...
  iApply (counter_full_run_spec with
           "[HPC Hidc $Hr1 $Hradv $Hrmap $Hinit $Hcode $Hadv $Hdat $Hna]")
  ; auto.
  1,2: solve_addr'.
