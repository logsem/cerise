...
Section cap_lang_rules.
  ...
  Lemma wp_jmpIEpair Ep pc_p pc_b pc_e pc_a r w mem regs  :
    decodeInstrW w = JmpIEpair r →
    isCorrectPC (WCap pc_p pc_b pc_e pc_a) →

    regs !! PC = Some (WCap pc_p pc_b pc_e pc_a) →
    allow_jmp_mmap_or_true cond_jmp r regs mem ->
    allow_jmp_rmap_or_true cond_jmp r regs ->
    regs_of (JmpIEpair r) ⊆ dom regs →
    mem !! pc_a = Some w →
  ...
  (* Lemma wp_jmpIEpair_success E pc_p pc_b pc_e pc_a w r w' : *)
  ...
  (* TODO we need to do more cases (for _automation_), where:
       ...
       - fail ?
   *)
  Lemma wp_jmp_success_IEpair E pc_p pc_b pc_e pc_a w r b e a w' wpc widc :
    decodeInstrW w = JmpIEpair r →
     isCorrectPC (WCap pc_p pc_b pc_e pc_a) →
     withinBounds b e a ->
     withinBounds b e (a^+1)%a ->
    ...
    destruct Hr' as (?&?&?).
    iApply (wp_jmpIEpair with "[$Hmem $Hreg]"); eauto ; simplify_map_eq; eauto.
  ...
  Lemma wp_jmp_success_IEpair_idc E pc_p pc_b pc_e pc_a w b e a wpc widc :
    decodeInstrW w = JmpIEpair idc →
     isCorrectPC (WCap pc_p pc_b pc_e pc_a) →
     withinBounds b e a ->
     withinBounds b e (a^+1)%a ->
  ...
  Lemma wp_jmp_fail_IEpair E pc_p pc_b pc_e pc_a w r b e a :
    decodeInstrW w = JmpIEpair r →
    isCorrectPC (WCap pc_p pc_b pc_e pc_a) →
    not (withinBounds b e a /\ withinBounds b e (a^+1)%a) ->
  ...
  Lemma wp_jmp_success_IEpair_same_idc E pc_p pc_b pc_e pc_a w b e a wpc widc :
    decodeInstrW w = JmpIEpair idc →
    isCorrectPC (WCap pc_p pc_b pc_e pc_a) →
    withinBounds b e a ->
    withinBounds b e (a^+1)%a ->
  ...
  Lemma wp_jmp_fail_IEpair_same_idc E pc_p pc_b pc_e pc_a w b e a :
    decodeInstrW w = JmpIEpair idc →
    isCorrectPC (WCap pc_p pc_b pc_e pc_a) →
    not (withinBounds b e a /\ withinBounds b e (a^+1)%a) ->
