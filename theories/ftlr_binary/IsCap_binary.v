...
From cap_machine.rules_binary Require Import rules_binary_base rules_binary_IsCap.
...
Section fundamental.
  ...
  Lemma IsCap_spec_determ r dst src regs regs' retv retv' :
    IsCap_spec r dst src regs retv ->
    IsCap_spec r dst src regs' retv' ->
  ...
  Lemma iscap_case (r : prodO (leibnizO Reg) (leibnizO Reg)) (p : Perm)
        (b e a : Addr) (w w' : Word) (dst : RegName) (src : RegName) (P : D):
    ftlr_instr r p b e a w w' (IsCap dst src) P.
  Proof.
    ...
    iApply (wp_IsCap with "[$Ha $Hmap]"); eauto.
    ...
    iMod (step_IsCap _ [SeqCtx] with "[$Ha' $Hsmap $Hs $Hspec]") as (retv' regs'') "(Hs' & Hs & Ha' & Hsmap) /=";[rewrite Heqw in Hi|..];eauto.
    ...
    specialize (IsCap_spec_determ _ _ _ _ _ _ _ HSpec HSpec') as [Hregs <-].
