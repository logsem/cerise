...
Section mutual_attest_A.
  ...
  Proof.
    ...
    iAssert (
        interp (LCap E pc_b pc_e (a_block2 ^+ 14)%a v)
      ) as "Hinterp_return"; first shelve.
    ...
    (* TODO  CALLBACK IN A LEMMA *)
    Transparent mutual_attest_enclave_A_code_pre encodeInstrsLW.
    (* ------------------ *)
    (* ----- RETURN ----- *)
    (* ------------------ *)
    Unshelve.
    iClear "Hinterp_map Hinterp_w0 Hinterp_w2 Hinterp_w1 Hjmp Hinterp_sealed_b'".
    clear dependent lregs w0 w1 w2 w3 w4 w5 w6.
    ...
    (* Lea r_t4 (-size_idT)%Z *)
    iInstr "Hma_code".
    { transitivity (Some (pc_e ^+ -2)%a); last done. admit. }
    (* Mov r_t3 r_t4 *)
    iInstr "Hma_code".
    (* Lea r_t3 offset_idB *)
    iInstr "Hma_code".
    { transitivity (Some (pc_e ^+ -1)%a); last done. admit. }
    (* Load r_t3 r_t3 *)
    replace (pc_e ^+ -1)%a  with (ma_addr_A ^+ (120%nat + 1))%a by admit.
    iInstr "Hma_code".
    { subst; solve_addr+He. }
    (* GetA r_t5 r_t4 *)
    iInstr "Hma_code".
    ...
    replace
      (hash_concat hash_mutual_attest_B_pre (hash [LInt hash_mutual_attest_A_pre; LInt hash_mutual_attest_B_pre]))
      with
    ...
    Transparent mutual_attest_enclave_A_code_pre encodeInstrsLW.
    replace x with (a_block3 ^+ 29)%a by admit.
    clear Z.
