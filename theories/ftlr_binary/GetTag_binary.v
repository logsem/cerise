...
From cap_machine.rules_binary Require Import rules_binary_base rules_binary_GetTag.
...
Section fundamental.
  ...
  Lemma GetTag_spec_determ r dst src regs regs' retv retv' :
    GetTag_spec r dst src regs retv ->
    GetTag_spec r dst src regs' retv' ->
  ...
  Lemma get_tag_case (r : prodO (leibnizO Reg) (leibnizO Reg)) (p : Perm)
        (b e a : Addr) (w w' : Word) (dst : RegName) (src : RegName) (P : D):
    ftlr_instr r p b e a w w' (GetTag dst src) P.
  Proof.
    ...
    iMod (step_GetTag _ [SeqCtx] with "[$Ha' $Hsmap $Hs $Hspec]") as (retv' regs'') "(Hs' & Hs & Ha' & Hsmap) /=";[rewrite Heqw in Hi|..];eauto.
    ...
    specialize (GetTag_spec_determ _ _ _ _ _ _ _ HSpec HSpec') as [Hregs <-].
