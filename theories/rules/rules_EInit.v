...
Section cap_lang_rules.
  ...
  Lemma wp_einit E pc_p pc_b pc_e pc_a pc_v pc_a' lw
    ...
    {{{ lregs' lmem' retv n' ot, RET retv;
        ...
        [∗ map] la↦lw ∈ lmem', la ↦ₐ lw ∗
        [∗ map] k↦y ∈ lregs', k ↦ᵣ y ∗
        EC⤇ n+1 ∗ (* need to give back the fragment of EC *)
  ...
  Proof.
    iIntros (Hinstr Hvpc Hpc Hdomi φ) "(Hregs & Hmem & Hpcc & Hpclw & Hec) Hφ".
    apply isCorrectLPC_isCorrectPC_iff in Hvpc; cbn in Hvpc.
    iApply wp_lift_atomic_head_step_no_fork; auto.
    iIntros (σ1 ns l1 l2 nt) "Hσ1 /=". destruct σ1; simpl.
    iDestruct "Hσ1" as (lr lm vmap) "(% & % & % & Hr & Hm & %Hcur & Henc_cur & Henc_prev & Henc_all & HEC & %Hdisj & %Hdom & %Hdisj2 & %Union_all & %Hstate)".
    iDestruct (@gen_heap_valid with "Hr Hpcc") as %?.
    iDestruct (@gen_heap_valid with "Hm Hpclw") as %?.
    iModIntro.
