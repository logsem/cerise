...
Section fundamental.
  ...
  Lemma GetWType_spec_determ r dst src regs regs' retv retv' :
    GetWType_spec r dst src regs retv ->
    GetWType_spec r dst src regs' retv' ->
  ...
  Lemma getwtype_case (r : prodO (leibnizO Reg) (leibnizO Reg)) (p : Perm)
        (b e a : Addr) (w w' : Word) (dst : RegName) (src : RegName) (P : D):
    ftlr_instr r p b e a w w' (GetWType dst src) P.
  Proof.
    ...
    iMod (step_GetWType _ [SeqCtx] with "[$Ha' $Hsmap $Hs $Hspec]") as (retv' regs'') "(Hs' & Hs & Ha' & Hsmap) /=";[rewrite Heqw in Hi|..];eauto.
    ...
    specialize (GetWType_spec_determ _ _ _ _ _ _ _ HSpec HSpec') as [Hregs <-].
