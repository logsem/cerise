...
(*  Lemma push_z_instrs_extract a i j z prog p' :
  ...
  Definition awkward_instrs (r1 : RegName) epilogue_off :=
     ...
     [load_r r1 r_env;
     move_r r_t1 PC;
     lea_z r_t1 61 (*62*); (* offset to assertion failure. for now this is the adress after the program *)
     sub_r_z r1 r1 1;
     jnz r_t1 r1] ++
  ...
  Lemma f4_spec W pc_p pc_g pc_b pc_e (* PC *)
    ...
    {{{ r_stk ↦ᵣ wstk
      ...
      ∗ interp W wret
      (* trusted code *)
      ∗ (∃ ι, na_inv logrel_nais ι (awkward_example f4_addrs pc_p r_adv (* 65 *) 40))
  ...
  Proof.
    ...
    iApply (pushU_r_spec); [..|iFrame "HPC Hpush Hr_stk Hr_env Ha"];
      [|apply PermFlows_refl|auto|iContiguous_next Hcont_rest0 1|iContiguous_next Hcont_rest0 1|auto|..].
    ...
    { iIntros (r1).
      ...
      - (* continuation *)
        ...
        iDestruct (mapsto_decomposition _ _ pc_p (take 85 (awkward_instrs r_adv 40)) with "Hprog")
        ...
        iApply (rules_StoreU_derived.wp_storeU_success_0_z with "[$HPC $Hi $Hr_stk $Hstack_own_end]");
          [apply storeU_z_z_i|apply PermFlows_refl|auto|iCorrectPC s_last a_last|iContiguous_next Hcont_rest1 1|
        ...
        iApply (wp_add_sub_lt_success_z_z with "[$HPC $Hr_t1 $Hinstr]");
          [apply sub_z_z_i|right;left;eauto|iContiguous_next Hcont_rest1 2|apply PermFlows_refl|iCorrectPC s_last a_last|..]. 
        ...
        iApply (wp_lea_success_reg with "[$HPC $Hr_t1 $Hr_stk $Hinstr]");
          [apply lea_r_i|apply PermFlows_refl|iCorrectPC s_last a_last|iContiguous_next Hcont_rest1 3| |auto..].
        ...
        iApply (popU_spec); [..|iFrame "HPC Hr_stk Hr_t1 Hpop Hr_adv Ha4"];
          [split;[|split];iCorrectPC s_last a_last| apply PermFlows_refl| econstructor|iContiguous_bounds_withinBounds a0 stack_own_last|auto|
        ...
        iApply (pushU_r_spec);[..|iFrame "HPC Hr_stk Hpush Hr_env Hb_r"];
          [iCorrectPC s_last a_last|apply PermFlows_refl|econstructor|auto|iContiguous_next Hcont_rest1 14|iContiguous_next Hcont_rest2 0|auto|..].
        iNext. iIntros "(HPC & Hpush & Hr_stk & Hr_env & Hb_r)". iCombine "Hpush" "Hprog_done" as "Hprog_done".
        (* push r_stk r_self *)
        (destruct rest1;[inversion Hrest_length|]).
        ...
        iApply (pushU_r_spec);[..|iFrame "HPC Hr_stk Hpush Hr_t0 Ha3"];
          [iCorrectPC s_last a_last|apply PermFlows_refl|econstructor|iContiguous_bounds_withinBounds a0 stack_own_last|iContiguous_next Hcont_rest1 15|..]. iContiguous_next Hcont_rest2 1.
        ...
        (* We don't need the ws_adv points to in the uninitialized case *)
        (* iAssert (∃ ws_adv : list Word, [[stack_own_end,estk]]↦ₐ[RWLX][[ws_adv]])%I with "[Hstack_adv Hlast']" as (ws_adv'') "Hstack_adv". *)
        ...
        (*   iDestruct (mapsto_decomposition _ _ _ [inr (RWLX, Local, a0, estk, stack_own_end)] with "[$Hstack_adv $Hlast']") as "Hstack_adv";[auto|..]. *)
        ...
        assert (∀ x, x ∈ ([stack_own_end] ++ ltempadv3) ->
                       (*([stack_own_end] ++ region_addrs stack_own_last estk) ->*)
        ...
        (* Need to turn the revoked into uninitialized instead *)
        (* iMod (monotone_close _ (region_addrs stack_own_end estk) RWLX (λ Wv, interp Wv.1 Wv.2) *)
        (*         with "[$Hsts $Hr Hstack_adv ]") as "[Hsts Hr]". *)
        ...
        iApply (wp_jmp_success with "[$Hinstr $Hr_adv $HPC]");
          [apply jmp_i|apply PermFlows_refl|iCorrectPC a22 a_last|..].
        iEpilogue "(HPC & Hinstr & Hr_adv)". iSimpl in "HPC".
        ...
        (*   iApply (adv_monotone with "Hadv_val").  *)
        (*   eapply related_sts_priv_trans_world;[|apply Hrelated5]. *)
        (*     by eapply related_sts_priv_pub_trans_world;[|apply Hrelated3]. } *)
        ...
        { iIntros (r1).
          ...
          - (* continuation *)
            ...
            iAssert ([∗ list] a_i;w_i ∈ ((((reqperm_prog ++ preptack_prog) ++ link0 :: a1 :: a5 :: a6 :: scall_prologue) ++
                     ...
                     (take 146 (awkward_instrs r_adv 40)) ++ (drop 146 (awkward_instrs r_adv 40)), a_i ↦ₐ[pc_p] w_i)%I
              with "[Hprog]" as "Hprog".
            ...
            assert (drop 146 (awkward_instrs r_adv 40) = [jmp r_adv;
                                                          sub_z_z r_t1 0 6;
                                                          lea_r r_stk r_t1] ++
                                                          popU_instrs r_stk r_t0 ++
                                                          popU_instrs r_stk r_env ++
            ...
            iApply (wp_add_sub_lt_success_z_z with "[$HPC Hr_t1 $Hinstr]");
              [apply sub_z_z_i|right;left;eauto|iContiguous_next Hcont_rest2 1|apply PermFlows_refl|iCorrectPC a22 a_last|
               iSimpl;iFrame;eauto|].
            iEpilogue "(HPC & Hinstr & Hr_t1)".
            iCombine "Hinstr" "Hprog_done" as "Hprog_done".
            (* lea r_stk r_t1 *)
            ...
            iApply (wp_lea_success_reg with "[$HPC $Hr_t1 $Hr_stk $Hinstr]");
              [apply lea_r_i|apply PermFlows_refl|iCorrectPC a22 a_last|
            ...
            iApply (popU_spec); [..|iFrame "HPC Hr_stk Hr_t1 Hpop Hr_t0 Ha3"];
              [split;[|split];iCorrectPC a22 a_last|apply PermFlows_refl|econstructor|iContiguous_bounds_withinBounds a0 stack_own_last|auto|iContiguous_next Hcont_rest2 3|iContiguous_next Hcont_rest2 4|iContiguous_next Hcont_rest2 5| |].
            ...
            iApply (popU_spec); [..|iFrame "HPC Hr_stk Hr_t1 Hpop Hr_env Ha2"];
              [split;[|split];iCorrectPC a22 a_last|apply PermFlows_refl|econstructor|iContiguous_bounds_withinBounds a0 stack_own_last|auto|iContiguous_next Hcont_rest2 6|iContiguous_next Hcont_rest2 7|iContiguous_next Hcont_rest2 8|iContiguous_next Hcont_rest2' 0|].
            ...
            { iIntros (φ') "Hφ'".
              ...
              iApply (wp_load_success with "[$HPC $Hinstr $Hr_adv $Hr_env Hb]");
                [apply load_r_i|apply PermFlows_refl|iCorrectPC a22 a_last
            ...
            iApply (wp_move_success_reg_fromPC with "[$HPC $Hr_t1 $Hinstr]");
              [apply move_r_i|apply PermFlows_refl|iCorrectPC a22 a_last|
            ...
            assert ((a32 + 61)%a = Some a_last) as Hincr.
            { revert Hcont_rest2 Hrest_length1. clear. intros Hcont_rest1 Hrest_length1.
              assert ((a22 + 10)%a = Some a32);[eapply contiguous_between_incr_addr with (i:=10) (ai:=a32) in Hcont_rest1;auto|].
              apply contiguous_between_length in Hcont_rest1. solve_addr. }
            iApply (wp_lea_success_z with "[$HPC $Hinstr $Hr_t1]");
              [apply lea_z_i|apply PermFlows_refl|iCorrectPC a22 a_last|
            ...
            { apply contiguous_between_length in Hcont_rest1.
              apply isCorrectPC_range_perm in Hvpc1; [|revert Hcont_rest1; clear; solve_addr].
            ...
            iApply (wp_add_sub_lt_success_dst_z with "[$HPC $Hinstr $Hr_adv]");
              [apply sub_r_z_i|right;left;eauto|iContiguous_next Hcont_rest2 12|apply PermFlows_refl|iCorrectPC a22 a_last|..]. 
            ...
            iApply (wp_jnz_success_next with "[$HPC $Hinstr $Hr_t1 $Hr_adv]");
              [apply jnz_i|apply PermFlows_refl|iCorrectPC a22 a_last|iContiguous_next Hcont_rest2 13|..].
            ...
            iApply (wp_Get_success with "[$HPC $Hinstr $Hr_stk $Hr_t1]");
              [apply getb_i|auto|apply PermFlows_refl|iCorrectPC a22 a_last|iContiguous_next Hcont_rest2 14|auto..].
            ...
            iApply (wp_add_sub_lt_success_r_z with "[$HPC $Hinstr $Hr_t2 $Hr_t1]");
              [apply add_r_z_i|by left|iContiguous_next Hcont_rest2 15|apply PermFlows_refl|iCorrectPC a22 a_last|..].
            ...
            iApply (wp_subseg_success with "[$HPC $Hinstr $Hr_stk $Hr_t1 $Hr_t2]");
              [apply subseg_r_r_i|apply PermFlows_refl|iCorrectPC a22 a_last|
            ...
            assert (contiguous_between (a39 :: rest2) a39 a_last) as Hcont_weak.
            ...
            assert (a39 < rclear_first)%a as Hcontlt2.
            { revert Hmclear_length Hlink3. clear. solve_addr. }
            assert (a22 <= a39)%a as Hcontge2.
            { apply region_addrs_of_contiguous_between in Hcont_scall1. subst.
              ...
              apply contiguous_between_middle_bounds with (i := 17) (ai := a39) in Hf2;[solve_addr|auto].
            }
            ...
            assert (a40 < jmp_addr)%a as Hcontlt3.
