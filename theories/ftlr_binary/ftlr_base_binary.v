 ...
 Context {Σ:gFunctors} {memg:memG Σ} {regg:regG Σ}
  ...
  Definition ftlr_instr (r : prodO (leibnizO Reg) (leibnizO Reg)) (p : Perm)
    ...
    → □ ▷ (∀ a0 a1 a2 a3 a4,
           ...
           -∗ registers_mapsto (<[PC:=inr (a1, a2, a3, a4)]> a0.1)
           -∗ spec_registers_mapsto (<[PC:=inr (a1, a2, a3, a4)]> a0.2)
           -∗ na_own logrel_nais ⊤
           -∗ ⤇ Seq (Instr Executable)
           -∗ □ spec_ctx
           -∗ □ interp (inr (a1, a2, a3, a4), inr (a1, a2, a3, a4)) -∗ interp_conf)
    -∗ spec_ctx
    ...
    -∗ ▷ □ (∀ w0 w'0 : Word, P (w0, w'0) -∗ interp (w0, w'0))
            ∗ (if decide (writeAllowed_in_r_a (<[PC:=inr (p, b, e, a)]> r.1) a) then ▷ □ (∀ w0 w'0 : Word, interp (w0, w'0) -∗ P (w0, w'0)) else emp)
    -∗ spec_registers_mapsto (<[PC:=inr (p, b, e, a)]> r.2)
    -∗ na_own logrel_nais ⊤
    -∗ ⤇ Seq (Instr Executable)
    -∗ a ↦ₐ w
    -∗ a ↣ₐ w'
    ...
    -∗ PC ↦ᵣ inr (p, b, e, a)
    -∗ ([∗ map] k↦y ∈ delete PC (<[PC:=inr (p, b, e, a)]> r.1), k ↦ᵣ y)
