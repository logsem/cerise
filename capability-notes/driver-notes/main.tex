...
\begin{itemize}
\item Define types for traces
\item Create a regular authoritative resource algebra over traces (we can refine this later on)
\item Parameterize everything by the MMIO = Set(Loc)? locations?
\item Formalize traces in the operational semantics $\rightarrow$ choose how we want to formalize MMIO with respect to memory
\item Formalize traces in the state interpretation
\end{itemize}
